...
| User allowlist | `~/.ai-security/mcp-allowlist.json`: written by the hooks when the user approves a server (native prompt matched by tool-call id, or exactly `approve <name>` in the chat for Codex); the gate passes allowlisted servers silently and re-prompts on a changed descriptor (command, arguments, URL, env, headers, cwd). A committed `.ai-security/mcp-allowlist.json` in a repo is honoured only after the user trusts it once. The agent cannot write either (gated) | Optionally ship a pre-filled copy with the MDM job; pair with each client's own MCP allowlist (§4), which is the preventive boundary a hostile agent cannot cross |
